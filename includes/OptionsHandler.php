...
class OptionsHandler extends SessionHandler {
	...
	public function clean( $maxlifetime, $next ) {
		...
		$keys   = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE %s AND option_value > %d ORDER BY option_value LIMIT 0, 1000',
				'wp_session_expires_%',
				$filter
			)
		);
